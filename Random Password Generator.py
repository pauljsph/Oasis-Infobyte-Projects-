...
class ModernBMICalculator:
    ...
    def create_input_field(self, parent, label_text, row):
        ...
        # Entry field with consistent dark styling
        entry = tk.Entry(
            parent,
            font=("Segoe UI", 13),
        ...
        entry.grid(row=row, column=1, sticky="ew", pady=(20, 8), ipady=8)
    ...
    def create_widgets(self):
        ...
        # Input card without the problematic border frame
        input_card = tk.Frame(self.main_frame, bg=self.colors['bg_card'], relief="flat")
        input_card.pack(fill="x", pady=(0, 25), padx=2)
        ...
        # Calculate button
        calc_btn = self.create_rounded_button(
            button_frame, "Calculate BMI", self.submit,
            self.colors['accent'], self.colors['accent_hover']
        )
        calc_btn.pack(side="left", expand=True, padx=(0, 10), fill="x")
        
        # History button
        history_btn = self.create_rounded_button(
        ...
        history_btn.pack(side="right", expand=True, padx=(10, 0), fill="x")
